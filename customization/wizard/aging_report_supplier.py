...
class aging_report_supplier(osv.osv_memory):
    _name = "aging.report.supplier"
    _columns = {
        'date_to': fields.date('To Date', required=True),
	'date_from': fields.date('From Date', required=True),
        'region': fields.many2one('res.region','Region'),
        'partner_id': fields.many2many('res.partner','aging_supplier_rel', 'partner_id', 'report_id', 'Supplier Name', domain=[('supplier','=','True')]),
    }
    ...
    def start_report(self, cr, uid, ids, data, context=None):
        for wiz_obj in self.read(cr,uid,ids):
            ...
            return {
                    ...
                    'report_name': 'aging_report_supplier',
                    'datas': data,
                    'nodestroy': True
                }
                

aging_report_supplier()
